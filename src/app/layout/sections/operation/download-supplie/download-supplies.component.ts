...
export class DownloadSuppliesComponent implements OnInit {
  ...
  funtionInit() {
    ...
    const promise2 = new Promise((resolve) => {
      this.serviceWorkspaces.GetDeliveriesToOperator().subscribe((response) => {
  ...
  closeDelivery(idDelivery) {
    this.serviceWorkspaces.CloseDelivery(idDelivery).subscribe((_) => {
  ...
  downloadReport(idSupplie: string, nameSupplie: string) {
    this.serviceWorkspaces
      .DownloadReportIndividual(this.IdEntrega, idSupplie)
      .subscribe((data: any) => {
        const contentType = data.headers.get('content-type');
        const type = contentType.split(',')[0];
        //const ext = data.headers.get('Content-Disposition');
